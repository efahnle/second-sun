...
def init_gpio(gpio_pin: int) -> tuple:
    ...
    log(f"Hardware PWM initialized on pin {gpio_pin} at 1000Hz")
...
def change_light_brightness(lamp_info: tuple, days: dict) -> None:
    ...
    if abs(brightness - change_light_brightness.last_brightness) > 0.01:  # Only update if >0.01% difference
        ...
        log(f"PWM: Changing brightness from {change_light_brightness.last_brightness:.2f}% to {rounded_brightness:.2f}% (duty={duty_cycle})")
        
        pi.hardware_PWM(gpio_pin, 1000, duty_cycle)
        change_light_brightness.last_brightness = rounded_brightness
        
        log(f"PWM: Change completed")
